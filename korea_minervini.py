...
# ============================================================
# K-Minervini v2.4 Relaxed Settings
# ============================================================
...
# V2.4 Relaxed quality thresholds
VCP_READY_SCORE = 4.5
...
def fmt_eok_signed(x):
    """
    원 단위 금액을 억원 단위 부호 포함 정수로 표시.
    예:
    12,340,000,000 -> +123
    -4,500,000,000 -> -45
    """
    try:
        if pd.isna(x):
            return "0"
        return f"{float(x) / 100_000_000:+.0f}"
    except Exception:
        return "0"
...
def get_net_purchase_by_investor(start_date, end_date, investor):
    ...
    merged[f"{investor}_net"] = (
        pd.to_numeric(merged[net_col], errors="coerce").fillna(0)
        if net_col
        else 0
    )
...
def detect_vcp(df):
    ...
    atr_ratio = (
        atr10 / atr30
        if not pd.isna(atr10) and not pd.isna(atr30) and atr30 > 0
        else np.nan
    )
...
def setup_scores(df):
    ...
    breakout_today = (
        close >= pivot
        and volume_explosion
        and turnover_explosion
        and vcp["vcp_score"] >= 3
    )
    ...
    prelim_stop = (
        max(recent_low, close * 0.95)
        if pullback_basic
        else max(recent_low, pivot * 0.94)
    )
    ...
    prelim_risk = (
        (prelim_entry - prelim_stop) / prelim_entry * 100
        if prelim_entry > 0
        else np.nan
    )
    ...
    pullback = bool(
        pullback_basic
        and pullback_quality["pullback_quality_score"] >= PULLBACK_QUALITY_READY_SCORE
    )
...
def analyze():
    ...
    print(f"🇰🇷 K-Minervini v2.4 Relaxed 실행일: {today_label}, 기준거래일: {end_date}")
    ...
    result.loc[
        (result["total_score"] >= BREAKOUT_TODAY_SCORE)
        & (result["breakout_today"]),
        "category",
    ] = "BREAKOUT_TODAY"
    ...
    result = result.sort_values(
        ["category", "total_score", "rs_percentile"],
...
def format_watch_table(df, title="👀 Watch 요약표", limit=30):
    """
    Watch 종목은 상세 설명 대신 표 형태로 요약합니다.
    외국인/기관 수급은 20일 누적 순매수 금액을 억원 단위로 표시합니다.
    """
    if df.empty:
        return f"{title}\n없음"
    ...
    lines.append("")
    lines.append("```")
    lines.append("종목          점수  RS  VCP  PB  Risk   수급20D        5D")
    lines.append("-------------------------------------------------------------")
    ...
    for _, r in show.iterrows():
        ...
        foreign_net = float(r.get("외국인_net", 0))
        inst_net = float(r.get("기관합계_net", 0))

        f_text = f"F{fmt_eok_signed(foreign_net)}"
        i_text = f"I{fmt_eok_signed(inst_net)}"
    ...
    lines.append("")
    lines.append("※ 수급20D: 외국인/기관 20일 누적 순매수, 억원")
    lines.append("※ 5D: 최근 5일 중 순매수 일수")
    lines.append("※ VCP 4.5 이상: 돌파대기 근접")
    lines.append("※ PB 5.5 이상: 눌림목 근접")
    lines.append("※ Risk 8% 초과 종목은 진입 보류")
...
def build_message(result, today_label, end_date, universe_count):
    ...
    msg = (
        ...
        f"{format_watch_table(watch, '👀 Watch 요약표', 30)}\n\n"
        f"{format_section(avoid, '⚠️ Avoid / 과열주의', 4)}\n\n"
        f"------------------------------------\n"
        f"※ 투자 추천이 아닌 자동 선별 결과입니다.\n"
        f"※ v2.4 Relaxed는 VCP 4.5, PB품질 5.5 기준으로 완화했습니다.\n"
        f"※ Watch는 수급 규모를 억원 단위 표로 표시합니다.\n"
...
def main():
    try:
        ...
        print("🎯 K-Minervini v2.4 Relaxed 완료")

    except Exception as e:
        print(traceback.format_exc())
        send_telegram_message(
            f"❌ K-Minervini v2.4 Relaxed 실행 실패\n"
            f"{e}\n\n"
