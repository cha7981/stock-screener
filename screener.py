...
def send_telegram_message(message):
  if not TELEGRAM_TOKEN or not CHAT_ID:
    print("❌ 에러: 텔레그램 환경변수(Secrets)가 설정되지 않았습니다.")
    return
  url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
  payload = {"chat_id": CHAT_ID, "text": message, "parse_mode": "Markdown"}
  try:
    res = requests.post(url, json=payload, timeout=10)
    print(f"📲 텔레그램 전송 상태코드: {res.status_code}")
...
def get_russell2000_tickers():
  try:
    df = pd.read_csv(
        "https://raw.githubusercontent.com/SergioIommi/Quant-Trading-Dashboards/master/RUT.csv"
    )
    tickers = (
        df["Symbol"].dropna().tolist()
        if "Symbol" in df.columns
        else df.iloc[:, 0].dropna().tolist()
    )
    return [str(t).strip().replace(".", "-") for t in tickers]
  except Exception as e:
    print(f"⚠️ 러셀 2000 수집 건너뜀 (사유: {e})")
    return []
  ...
  if spy_close_series is None or spy_close_series.empty:
    ...
    try:
      ...
      combined = pd.DataFrame(
          {"Stock": stock_close, "SPY": spy_close_series}
      ).dropna()
      if len(combined) < 50:
        cond_rs = False
      else:
...
if __name__ == "__main__":
  ...
  if not tickers:
    print("❌ 에러: 종목 명단 수집 실패로 프로그램을 종료합니다.")
  else:
    stage1_count = 0
    stage3_vcp_details = []
    ...
    for i, ticker_symbol in enumerate(tickers):
      ...
      if (i + 1) % 100 == 0 or (i + 1) == len(tickers):
        print(
            f"🔄 스캔 진행 중... ({i + 1}/{len(tickers)}) | 현재 1차 통과:"
            f" {stage1_count}개"
        )
    ...
    t3_text = (
        "\n".join([
            ...
            for item in stage3_vcp_details
        ])
        if stage3_vcp_details
        else "조건을 만족하는 매수 임박 종목이 없습니다."
    )

    msg = (
        f"🔔 *[{today_str}] 미너비니 스크리닝 결과*\n"
        "------------------------------------\n"
        f"📊 *대상:* S&P500+나스닥100+러셀2000 (총 {len(tickers)}개)\n"
        f"📈 *1차 추세+거래량 필터 통과:* 총 {stage1_count}개\n\n"
        "🔥 *최종 VCP 통과*\n"
        f"{t3_text}\n"
        "------------------------------------"
    )
